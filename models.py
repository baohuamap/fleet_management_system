...
class Vehicle(Base):
    ...
    vehicle_plate_number = Column(String, index=True, nullable=False, unique=True)
    date_created = Column(TIMESTAMP(timezone=True),
                          nullable=False, server_default=text('now()'))
    owner_id = Column(Integer, ForeignKey(
        "fleets.id", ondelete="CASCADE"), nullable=False)
...
class Route(Base):
    ...
    driver_id = Column(Integer, ForeignKey(
        "drivers.id", ondelete="CASCADE"), nullable=False)
