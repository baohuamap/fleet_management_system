...
async def get_all_vehicles_in_fleet(
    ...
    vehicles = db.query(models.Vehicle).filter(
        models.Vehicle.owner_id == fleet_id
    ).all()
    return list(map(schemas.Vehicle.model_validate, vehicles))


async def get_vehicle(
    vehicle_id: int, db: "Session"
) -> models.Vehicle:
...
async def delete_vehicle(
    vehicle: models.Vehicle, db: "Session"
):
    db.delete(vehicle)
    db.commit()
...
async def delete_driver(
    driver: models.Driver, db: "Session"
):
    db.delete(driver)
    db.commit()
